...
export function SourceHealthCards({ pendingCount }: SourceHealthCardsProps) {
  ...
  const checkHealth = async () => {
    ...
    try {
      const res = await fetch("http://localhost:3002/health", { signal: AbortSignal.timeout(1500) });
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8 select-none">
      ...
      <div className="rounded-2xl border border-white/10 dark:border-white/5 bg-gradient-to-b from-card to-card/50 p-5 shadow-xs flex flex-col justify-between min-h-[110px] hover:shadow-md transition-all duration-300">
        <div>
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Triage Queue</span>
            <span className="h-1.5 w-1.5 rounded-full bg-indigo-500 animate-ping"></span>
          </div>
          <p className="text-2xl font-extrabold tracking-tight text-foreground">{pendingCount} Pending</p>
        </div>
        <p className="text-[10px] text-muted-foreground font-mono mt-2">Awaiting calendar block confirmation</p>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 dark:border-white/5 bg-gradient-to-b from-card to-card/50 p-5 shadow-xs flex flex-col justify-between min-h-[110px] hover:shadow-md transition-all duration-300">
        <div>
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Channels Sync State</span>
          ...
          <p className={[
            ...
            {slackStatus === "online" || emailStatus === "online" || calendarStatus === "online" ? "Active Connection" : "Offline"}
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-white/10 dark:border-white/5 bg-gradient-to-b from-card to-card/50 p-5 shadow-xs flex flex-col justify-between min-h-[110px] hover:shadow-md transition-all duration-300">
        <div>
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">AI Swarm Engine</span>
            {swarmStatus === "online" ? <Cpu className="h-3.5 w-3.5 text-indigo-500" /> : <ShieldAlert className="h-3.5 w-3.5 text-rose-500" />}
          </div>
          <p className={[
            "text-2xl font-extrabold tracking-tight",
        ...
        <p className="text-[10px] text-muted-foreground font-mono mt-2">
          {swarmStatus === "online" ? "Swarm resolving threshold at 90% cert" : "Swarm offline, using simulation fallbacks"}
        </p>
      </div>
    </div>
  );
