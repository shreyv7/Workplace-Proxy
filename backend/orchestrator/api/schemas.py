...
class SynthesisPreviewRequest(BaseModel):
    message: str = Field(..., description="Vague corporate message to translate")
    format: str = Field(..., description="checklist | summary | bullets | paragraph")
    verbosity: str = Field(..., description="minimal | balanced | detailed")


class SynthesisPreviewResponse(BaseModel):
    synthesized_text: str = Field(..., description="The translated text generated by the agent swarm")
    simulated: bool = Field(default=False, description="True if a simulated fallback was used due to missing API key")
